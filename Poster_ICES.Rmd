...
per.MSE<-rbind(obj.dataTable,altcat.data) %>% 
  mutate(order=c(2,1,4,3)) %>% 
  arrange(order) %>% 
  select(Type,Analysis,'Per MSE')
...
Map of where reviewed MSEs and climate change focues MSEs occurred: Figure \@ref(fig:mapfigure).

```{r, mapfigure, fig.cap='Map of MSE study locations. Here is a caption for the figure. This can be added by using the "fig.cap" option in the r code chunk options, see this [link](https://yihui.name/knitr/options/#plots) from the legend himself, [Yihui Xie](https://twitter.com/xieyihui).', out.width="100%"}
MSE.map
```

Figure \@ref(fig:explicit)
...
A greater number of objectives were considered per MSE in the random sample of reviewed MSEs than in the climate change MSEs (Table \@ref(tab:PerMSE))
...
per.MSE %>%
  kable(align = "c", caption = "Objective data reviewed") %>%  
  row_spec(color_CC, color = "#FF0033", background = "white") %>%
  row_spec(color_Pub, color = "#0000FF", background = "white")
```
